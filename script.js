document.addEventListener('DOMContentLoaded',()=>{
  ...
  style.textContent=`
    ...
    .desktop-nav a,.nav-cta,.brand{font-family:'Manrope',sans-serif}.desktop-nav a{font-size:11px!important;color:#5a6961!important}.brand{font-size:14px!important}
    ...
    .hero h1{font-size:clamp(68px,7.65vw,116px)!important;line-height:.80!important;letter-spacing:-.078em!important}.lead{font-family:'Manrope',sans-serif!important;font-size:23px!important}
    ...
    .ingredient{box-shadow:0 15px 45px rgba(40,57,46,.07)}.ingredient:after{background:linear-gradient(0deg,rgba(17,27,22,.48),rgba(17,27,22,.02) 62%)!important}
    .contact input,.contact select,.contact textarea{background:rgba(231,238,232,.64)!important;backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.reveal{opacity:1!important;transform:none!important}
    /* Reference screenshot assets are complete card compositions, so do not add duplicate labels. */
    .ingredient>div{display:none!important}.hero-photo .photo-label{display:none!important}
    .hero-photo img,.ingredient img{object-fit:fill!important}
  ...
  const assets={
    '.hero-photo img':'hero-reference.jpg',
    '.ingredient.garlic img':'garlic-reference.jpg',
    '.ingredient.turmeric img':'turmeric-reference.jpg',
    '.ingredient.neem img':'neem-reference.jpg',
    '.ingredient.moringa img':'moringa-reference.jpg',
    '.ingredient.tulsi img':'tulsi-reference.jpg'
  };
  Object.entries(assets).forEach(([selector,file])=>{const el=document.querySelector(selector);if(el){el.src=root+file;el.removeAttribute('srcset');}});

  const nav=document.querySelector('.desktop-nav'),menu=document.querySelector('.menu');
  ...
  document.querySelectorAll('a[href^="#"]').forEach(a=>a.addEventListener('click',e=>{const id=a.getAttribute('href'),target=document.querySelector(id);if(!target)return;e.preventDefault();window.scrollTo({top:target.getBoundingClientRect().top+window.scrollY-82,behavior:'smooth'});history.replaceState(null,'',id)}));
});
